...
public class ElfInspector extends Plugin {
    ...
    class InspectionRunner extends Thread {
        String filename;
        public InspectionRunner(String f) {
            filename = f;
        }
        ...
        public void run() {
            try {
                ...
                ctx.heading("Parsing ELF file");

                elf = new RandomAccessFile(filename, "r");
                if (elf == null) {
                    ctx.error("Unable to open " + filename);
                    return;
                }

                ctx.bullet("Opening ELF file");
    ...
    public void inspectFile(String filename) {
        InspectionRunner thread = new InspectionRunner(filename);
        thread.start();
    }
