...
//    }
  ...
  public String build(String buildPath) {
    ...
    try {
        primaryClassName = preprocess(buildPath);
    } catch (Exception e) {
    ...
    editor.status.progressNotice("Build Failed!");
    return null;
  }
  ...
  protected void size(String buildPath, String suggestedClassName)
    throws RunnerException {
    long size = 0;
    String maxsizeString = Base.selectedBoard.get("upload.maximum_size");
    String maxramString = Base.selectedBoard.get("upload.maximum_ram", "0");
    if (maxsizeString == null) return;
    long maxsize = Integer.parseInt(maxsizeString);
    long maxram = Integer.parseInt(maxramString);
    Sizer sizer = new Sizer(buildPath, suggestedClassName);
      try {
      size = sizer.computeSize();
        
        System.out.println("Sketch size:");
        System.out.println("  Program Memory: " + ((size * 100) / maxsize) + 
            "% (" + size + " of " + maxsize + " bytes)");
        if (maxram > 0) {
            System.out.println("  RAM Memory:     " + ((sizer.ramSize() * 100) / maxram) + 
                "% (" + sizer.ramSize() + " of " + maxram + " bytes)");
        }
  ...
  public boolean isReadOnly() {
    ...
    String me = Base.getContentFile(null).getAbsolutePath();
    if (apath.startsWith(me) ||
